...
SoundStream::~SoundStream()
{
    ...
    {
        std::lock_guard<std::mutex> lg(m_threadMutex);
        m_isStreaming = false;
    }
...
void SoundStream::play()
{
    ...
    assert(m_thread == nullptr);
    m_thread = new std::thread(&SoundStream::streamData, this);
}
...
////////////////////////////////////////////////////////////
SoundStream::Status SoundStream::getStatus()
{
...
void SoundStream::setPlayingOffset(Time timeOffset)
{
    ...
    // Restart streaming
    m_samplesProcessed = static_cast<audio::Uint64>(timeOffset.asSeconds() * m_sampleRate * m_channelCount);
...
////////////////////////////////////////////////////////////
audio::Int64 SoundStream::onLoop()
{
    onSeek(Time::Zero);
    return 0;
}
...
bool SoundStream::fillAndPushBuffer(unsigned int bufferNum, bool immediateLoop)
{
    ...
    Chunk data = {NULL, 0};
    for (audio::Uint32 retryCount = 0; !onGetData(data) && (retryCount < BufferRetries); ++retryCount)
    {
    ...
    if (data.samples && data.sampleCount)
    {
        ...
        // Fill the buffer
        ALsizei size = static_cast<ALsizei>(data.sampleCount) * sizeof(audio::Int16);
