...
// Headers
////////////////////////////////////////////////////////////
#include "SoundFileWriterWav.hpp"
#include "Err.hpp"
#include <algorithm>
#include <cctype>
#include <cassert>
...
namespace
{
    ...
    void encode(std::ostream& stream, audio::Int16 value)
    {
        unsigned char bytes[] =
        {
    ...
    void encode(std::ostream& stream, audio::Uint16 value)
    {
        unsigned char bytes[] =
        {
            static_cast<unsigned char>(value & 0xFF),
    ...
    void encode(std::ostream& stream, audio::Uint32 value)
    {
        unsigned char bytes[] =
        {
...
////////////////////////////////////////////////////////////
void SoundFileWriterWav::write(const audio::Int16* samples, audio::Uint64 count)
{
    assert(m_file.good());
...
bool SoundFileWriterWav::writeHeader(unsigned int sampleRate, unsigned int channelCount)
{
    ...
    audio::Uint32 mainChunkSize = 0; // placeholder, will be written later
    ...
    audio::Uint32 fmtChunkSize = 16;
    encode(m_file, fmtChunkSize);

    // Write the format (PCM)
    audio::Uint16 format = 1;
    encode(m_file, format);

    // Write the sound attributes
    encode(m_file, static_cast<audio::Uint16>(channelCount));
    encode(m_file, static_cast<audio::Uint32>(sampleRate));
    audio::Uint32 byteRate = sampleRate * channelCount * 2;
    encode(m_file, byteRate);
    audio::Uint16 blockAlign = channelCount * 2;
    encode(m_file, blockAlign);
    audio::Uint16 bitsPerSample = 16;
    ...
    audio::Uint32 dataChunkSize = 0; // placeholder, will be written later
...
void SoundFileWriterWav::close()
{
    ...
    if (m_file.is_open())
    {
        ...
        audio::Uint32 fileSize = static_cast<audio::Uint32>(m_file.tellp());
        audio::Uint32 mainChunkSize = fileSize - 8;  // 8 bytes RIFF header
        audio::Uint32 dataChunkSize = fileSize - 44; // 44 bytes RIFF + WAVE headers
        m_file.seekp(4);
        encode(m_file, mainChunkSize);
        m_file.seekp(40);
        encode(m_file, dataChunkSize);
