...
namespace
{
    ...
    bool decode(sf::InputStream& stream, audio::Uint8& value)
    {
         return stream.read(&value, sizeof(value)) == sizeof(value);
    }

    bool decode(sf::InputStream& stream, audio::Int16& value)
    {
    ...
    bool decode(sf::InputStream& stream, audio::Uint16& value)
    {
        unsigned char bytes[sizeof(value)];
        if (stream.read(bytes, sizeof(bytes)) != sizeof(bytes))
            return false;
    ...
    bool decode24bit(sf::InputStream& stream, audio::Uint32& value)
    {
    ...
    bool decode(sf::InputStream& stream, audio::Uint32& value)
    {
        unsigned char bytes[sizeof(value)];
        if (stream.read(bytes, sizeof(bytes)) != sizeof(bytes))
            return false;
    ...
    const audio::Uint64 mainChunkSize = 12;

    const audio::Uint16 waveFormatPcm = 1;

    const audio::Uint16 waveFormatExtensible= 65534;
...
bool SoundFileReaderWav::check(InputStream& stream)
{
    ...
    if (stream.read(header, sizeof(header)) < static_cast<audio::Int64>(sizeof(header)))
        return false;
...
////////////////////////////////////////////////////////////
void SoundFileReaderWav::seek(audio::Uint64 sampleOffset)
{
    assert(m_stream);
...
////////////////////////////////////////////////////////////
audio::Uint64 SoundFileReaderWav::read(audio::Int16* samples, audio::Uint64 maxCount)
{
    assert(m_stream);

    audio::Uint64 count = 0;
    audio::Uint64 startPos = m_stream->tell();
    ...
    while ((count < maxCount) && (startPos + count * m_bytesPerSample < m_dataEnd))
    {
        switch (m_bytesPerSample)
        {
            case 1:
            {
                audio::Uint8 sample = 0;
                if (decode(*m_stream, sample))
                    *samples++ = (static_cast<audio::Int16>(sample) - 128) << 8;
                else
                    return count;
                break;
            }
...
bool SoundFileReaderWav::parseHeader(Info& info)
{
    ...
    while (!dataChunkFound)
    {
        ...
        if (m_stream->read(subChunkId, sizeof(subChunkId)) != sizeof(subChunkId))
            return false;
        audio::Uint32 subChunkSize = 0;
        if (!decode(*m_stream, subChunkSize))
            return false;
        audio::Int64 subChunkStart = m_stream->tell();
        if (subChunkStart == -1)
            return false;
        ...
        if ((subChunkId[0] == 'f') && (subChunkId[1] == 'm') && (subChunkId[2] == 't') && (subChunkId[3] == ' '))
        {
            ...
            // Channel count
            audio::Uint16 channelCount = 0;
            if (!decode(*m_stream, channelCount))
                return false;
            info.channelCount = channelCount;

            // Sample rate
            audio::Uint32 sampleRate = 0;
            if (!decode(*m_stream, sampleRate))
                return false;
            info.sampleRate = sampleRate;

            // Byte rate
            audio::Uint32 byteRate = 0;
            if (!decode(*m_stream, byteRate))
                return false;

            // Block align
            audio::Uint16 blockAlign = 0;
            if (!decode(*m_stream, blockAlign))
                return false;

            // Bits per sample
            audio::Uint16 bitsPerSample = 0;
            ...
            {
                // Extension size
                audio::Uint16 extensionSize = 0;
                if (!decode(*m_stream, extensionSize))
                    return false;

                // Valid bits per sample
                audio::Uint16 validBitsPerSample = 0;
                if (!decode(*m_stream, validBitsPerSample))
                    return false;

                // Channel mask
                audio::Uint32 channelMask = 0;
